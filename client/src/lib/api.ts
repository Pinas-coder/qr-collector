import type { AnteprimaPuntoInteresse, PuntoInteresse, ProfiloUtente } from "../../../shared/types";
...
export async function getPuntiInteresse(): Promise<AnteprimaPuntoInteresse[]> { const res = await fetch(`${BASE}/pois`, { headers: headers() }); if (!res.ok) throw await errorFrom(res, "Impossibile caricare i punti di interesse"); return res.json(); }
